...
void OperationSelect(void)
{
    ...
    printf("   %d: Load Linux then write to Flash via USB Storage. \n", 5);
	printf("   b: Load Uboot then write to Flash via USB Storage. \n");
}
...
/*config bootdelay via environment parameter: bootdelay */
	...
	if (!reg) {
		...
		while (!(RALINK_REG(RALINK_PIO_BASE+PIO_DATA1) & (1 << 6)))
		{
			...
			if (timer1 == 3) {
				RALINK_REG(RALINK_PIO_BASE+PIO_SET1) |= (1 << 12);
                printf ("IO38 low for 3s.\n");
            }
			if (timer1 == 20) {
				RALINK_REG(RALINK_PIO_BASE+PIO_CLEAR1) |= (1 << 12);
                printf ("IO38 low for 20s.\n");
            }
		}
